...
//
// All inputs and outputs are on the original Glicko-1 scale (r, RD).
// Internal computations use the Glicko-2 scale (μ, φ, σ) where:
//
//	μ = (r − 1500) / 173.7178
//	φ = RD / 173.7178
//	σ = volatility (same in both scales)
//
// 173.7178 = 400 / ln(10)  — the Glicko-2 scale constant.

const (
	glicko2Scale     = 173.7178 // (400 / ln(10)) — converts Glicko-1 ↔ Glicko-2
	glickoOpponentRD = 50.0     // residual uncertainty for a single session as opponent (Glicko-1 scale)
)

type GlickoSessionInput struct {
	PlayerRating      float64 // Glicko-1 scale
	PlayerRD          float64 // Glicko-1 scale
	PlayerVol         float64 // volatility (same on both scales)
	AvgOpponentRating float64 // Glicko-1 scale
	Score             float64 // 0.0–1.0 (session accuracy)
}

type GlickoOutput struct {
	NewRating float64 // Glicko-1 scale
	NewRD     float64 // Glicko-1 scale
	NewVol    float64 // volatility
}
...
func UpdateGlickoFromSession(in GlickoSessionInput) GlickoOutput {
	...
	// ── Convert inputs to Glicko-2 scale ─────────────────────────────────
	r := in.PlayerRating
	...
	// ── Glicko-2 update (single opponent = the whole session) ─────────────
...
// glicko2G computes g(φ) = 1 / sqrt(1 + 3·φ²/π²) on Glicko-2 scale.
func glicko2G(phi float64) float64 {
	return 1.0 / math.Sqrt(1.0+3.0*phi*phi/(math.Pi*math.Pi))
}

// glicko2Vol computes the new volatility σ' using the Illinois algorithm.
// Input phi is on Glicko-2 scale; v is the variance from step 4.
